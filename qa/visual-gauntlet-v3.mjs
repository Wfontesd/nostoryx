  ...
  return page.evaluate(({ sceneKey, requiredFrames }) => {
    ...
    return {
      ...
      state: scene ? {
        ...
        level: typeof scene.level === 'number' ? scene.level : null,
...
async function press(page, key, delay = 100, settle = 150) {
...
const scenarios = [
  { name: 'hub-desktop', scene: 'LabHub', query: '', viewport: DESKTOP, run: async ({ page, captures }) => captures.push(await shot(page, 'hub-desktop')) },
  {
    ...
    run: async ({ page, before, captures, failures }) => {
      ...
      await page.evaluate(() => {
        const scene = globalThis.__NOSTORYX_DEV_GAME__.scene.getScene('MovementLab');
        scene.player.body.setVelocityY(-scene.controller.jumpSpeed);
        scene.events.emit('player-jump', scene.player.x, scene.player.y);
      });
      await page.waitForTimeout(380);
      captures.push(await shot(page, 'movement-course'));
      const after = await inspect(page, 'MovementLab');
      check((after.state?.playerX ?? 0) > (before.state?.playerX ?? 0) + 250, 'controller did not traverse the scene while right input was held', failures);
      return after;
    },
  },
  {
    ...
    run: async ({ page, captures, failures }) => {
      ...
      let maxCombo = 0;
      await positionCombatant(page); await press(page, 'j', 90, 45); maxCombo = Math.max(maxCombo, (await inspect(page, 'CombatLab')).state?.combo ?? 0); captures.push(await shot(page, 'combat-light-impact'));
      await page.waitForTimeout(300);
      await positionCombatant(page); await press(page, 'k', 90, 110); maxCombo = Math.max(maxCombo, (await inspect(page, 'CombatLab')).state?.combo ?? 0); captures.push(await shot(page, 'combat-heavy-impact'));
      await page.waitForTimeout(730);
      await positionCombatant(page); await press(page, 'l', 90, 180); maxCombo = Math.max(maxCombo, (await inspect(page, 'CombatLab')).state?.combo ?? 0); captures.push(await shot(page, 'combat-arc-surge'));
  ...
  },
  {
    ...
    run: async ({ page, captures, failures }) => {
      ...
      await page.evaluate(() => {
        ...
        for (let i = 0; i < 4; i += 1) scene.damagePulse();
        scene.levelUp();
      });
...
try {
  for (const scenario of scenarios) {
    ...
    try {
      ...
      for (const capture of captures) check(capture.bytes > 45_000, `${capture.file} is suspiciously small (${capture.bytes} bytes)`, failures);
